...
export async function addToCart(productId: string, vendorId: string, quantity: number = 1) {
  try {
    ...
    console.log("addToCart server action started:", { productId, vendorId, userId: user?.id });
    ...
    if (existingItem) {
      ...
      console.log(`[addToCart] Updating existing item qty: ${existingItem.quantity} -> ${newQty}`);
      ...
      console.log(`[addToCart] Inserting new item: ${productId}`);
    ...
    console.log(`[addToCart] Updating order totals: ${newTotal}`);
