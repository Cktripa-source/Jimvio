...
import { Heart, MessageCircle, Share2, MoreHorizontal, Pin, Loader2, Send } from "lucide-react";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import { Button } from "@/components/ui/button";
import { timeAgo } from "@/lib/utils";
import { likePost, createPostComment } from "@/lib/actions/community";
...
export function PostCard({ post, isMember }: PostCardProps) {
  ...
  return (
    <div className="bg-white/80 backdrop-blur-xl border border-white/20 rounded-3xl shadow-sm hover:shadow-xl transition-all duration-500 overflow-hidden group">
      ...
      <div className="p-6 pb-0">
        ...
        {/* Post Body */}
        {post.title && (
          <h3 className="text-lg font-black text-[var(--color-text-primary)] mb-2 leading-tight">{post.title}</h3>
        )}
      ...
      {/* Actions */}
      <div className="px-6 py-3 border-t border-[var(--color-border)]/50 flex items-center gap-1">
        <button
          onClick={handleLike}
          className={cn(
        ...
        <button className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold text-[var(--color-text-muted)] hover:text-[var(--color-accent)] hover:bg-[var(--color-accent-light)] transition-all ml-auto">
