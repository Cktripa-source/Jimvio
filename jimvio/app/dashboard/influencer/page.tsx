...
import { 
  ...
  ShoppingBag, Star, LayoutDashboard, Globe
} from "lucide-react";
...
export default function InfluencerDashboardPage() {
  ...
  const [stats, setStats]           = useState({ totalEarnings: 0, totalClicks: 0, totalConversions: 0, activeCampaigns: 0 });
  ...
  useEffect(() => {
    async function load() {
      ...
      if (inf) {
        // Mock stats or load from actual tables if they exist
        setStats({
          totalEarnings: inf.total_earnings || 0,
          totalClicks: 1248, // Mock
          totalConversions: inf.total_conversions || 42,
          activeCampaigns: 3,
        });
  ...
  if (!influencer) {
    return (
      <div className="bg-subtle border border-base rounded-2xl p-12 text-center max-w-2xl mx-auto mt-10">
        <Video className="h-16 w-16 text-[var(--color-accent)] mx-auto mb-6" />
        <h2 className="text-2xl font-black text-[var(--color-text-primary)] mb-4">Launch Your Creator Career</h2>
        <p className="text-[var(--color-text-secondary)] mb-8">
          Activate your Influencer role to browse campaigns, download viral clips, and start earning commissions by sharing products with your audience.
        </p>
        <Button size="lg" asChild className="font-black rounded-xl px-10">
          <Link href="/dashboard/roles">Activate Influencer Role</Link>
        </Button>
      </div>
    );
  }
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-muted-c mt-1">Grow your audience and your income at once.</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Total Earnings" value={formatCurrency(stats.totalEarnings)} icon={<DollarSign className="h-4 w-4" />} iconColor="from-emerald-600 to-teal-600" />
        <StatCard title="Link Clicks" value={stats.totalClicks.toLocaleString()} icon={<MousePointer className="h-4 w-4" />} iconColor="from-blue-600 to-cyan-600" />
        <StatCard title="Conversions" value={stats.totalConversions} icon={<Zap className="h-4 w-4" />} iconColor="from-amber-600 to-orange-600" />
        <StatCard title="Active Campaigns" value={stats.activeCampaigns} icon={<Star className="h-4 w-4" />} iconColor="from-pink-600 to-rose-600" />
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <CardContent>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {recentClips.map((clip) => (
                  <div key={clip.id} className="group relative aspect-video rounded-2xl overflow-hidden bg-black border border-[var(--color-border)]">
                    <img 
        ...
        <div className="space-y-6">
          <Card className="bg-[#1a1a1a] text-white border-none shadow-xl relative overflow-hidden group">
            ...
            <CardContent>
              ...
              <Button className="w-full bg-white text-black hover:bg-white/90 font-black rounded-xl">
